...
{
    /// <summary>
    /// Response class of "CheckDeviceID" call. 
    /// Returns the existens of the AppID in the variable "Exists"
    /// Also returns CallStatus that is Success or Error.
    /// Errors could be found in the Errors variable.
    /// </summary>
    public class PostboxCheckDeviceIdResponse : PostboxResponse
    {
        /// <summary>
        /// Return if the device exists on api
        /// </summary>
        ...
        public PostboxCheckDeviceIdResponse(string localRequestId, PostboxCallName callName, XmlDocument response) : base(localRequestId, callName, response)
        {
            if (response == null)
                return;
        ...
        /// <param name="callName">callname of request</param>
        /// <param name="response">response object.</param>
        public PostboxCheckDeviceIdResponse(string localRequestId, PostboxCallName callName, JSONObject response) : base(localRequestId, callName, response)
        {
            if (response == null)
                return;
