...
/// </summary>
public class UIBubbleDrawer : MonoBehaviour 
{
	...
	private void Start() 
	{
        ...
        meshRenderer = GetComponent<RawImage>();
	}

    public void SetSplashTexture(int index)
    { 
        meshRenderer.texture = splashs[index];  
        SetAlpha(1.0f); 
        ResetMask();
    }
	...
	{
        stencialMap = new Texture2D(meshRenderer.texture.width, meshRenderer.texture.height);
	...
	private void PredefineTheMask()
	{
        ...
        Color32[] mainCols = (meshRenderer.texture as Texture2D).GetPixels32();
