/// <summary>
/// This is a generic class that you can use for saving/loading your data :)
/// </summary>
...
public class DataStorage
{
    ...
    public static void SaveToFile<T>(T dataToStore, StorageMethod method, string filename = "SaveGame")
	{
        string fileName = SavingLocation + filename + "." + method.ToString().ToLower();
	...
	/// <param name="method">Storing method</param>
	/// <typeparam name="T">Typeof data to store</typeparam>
    public static T LoadFromFile<T>(StorageMethod method, string filename = "SaveGame")
	{
	...
	/// </summary>
    public static void RemoveData(string filename = "SaveGame")
	{
		for(int i = 0; i < System.Enum.GetNames(typeof(StorageMethod)).Length; i++)
		{
            string fileName = SavingLocation + filename + "." + ((StorageMethod)i).ToString().ToLower();
